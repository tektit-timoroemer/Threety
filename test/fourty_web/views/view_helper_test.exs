defmodule Fourty.ViewHelperTest do
	...
	describe "my_text_input/_value provides text fields" do
		...
		test "my_text_value" do
			...
			assert l == ~s'<input class="form-control" type="text" ' <>
		...
		test "my_text_input" do
			...
			assert l == ~s'<input class="form-control is-valid" ' <>
			...
			assert l == ~s'<input class="form-control is-invalid" ' <>
		...
		test "my_text_link creates text field with link" do
			...
			assert l == ~s'<div class="d-grid gap-2">' <>
		...
		test "my_longtext_input" do
			...
			assert l == ~s'<textarea class="form-control is-valid" ' <>
			...
			assert l == ~s'<textarea class="form-control is-invalid" ' <>
		...
		test "my_longtext_value" do
			...
			assert l == ~s'<textarea class="form-control" disabled readonly>' <>
				~s'Test 123</textarea>'
		end
		...
		test "my_checkbox_input" do
			...
			assert l == ~s'<input name="project[visible]" type="hidden" ' <>
		...
		test "my_checkbox_value" do
			...
			assert l == ~s'<input class="form-check-input" type="checkbox" ' <>
